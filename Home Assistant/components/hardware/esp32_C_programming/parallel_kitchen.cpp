...
void taskSendData(void *parameter) {
  for (;;) { // Vòng lặp vô tận cho tác vụ
    ...
    // Gửi nhiệt độ
    if(Firebase.setFloat(fbDataSend, "/kitchen/sensor/temperature", temp)) {
    ...
    // Gửi độ ẩm
    if(Firebase.setFloat(fbDataSend, "/kitchen/sensor/humidity", humi)) {
...
void taskReceiveData(void *parameter) {
  for (;;) { // Vòng lặp vô tận cho tác vụ
    ...
    if(Firebase.getString(fbDataReceive, "/kitchen/device/lights")) {
    ...
    // Điều khiển LED PWM
    if(Firebase.getInt(fbDataReceive, "/kitchen/device/lamp")) {
